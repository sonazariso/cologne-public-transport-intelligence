...
| Raw GTFS | `/Users/admin/Documents/DataBaseKÖLN/google_transit_goR/` | Documented macOS source path; checked during this audit | No direct Windows mapping was verified; SQL Server loads only from a Windows service-readable path | No; raw GTFS is outside Git | External local source; no repository synchronization | Directory does not currently exist at the documented path. Raw GTFS was not added to Git. |
